...
public class GT_GUIContainer_MultiMachine extends GT_GUIContainerMetaTile_Machine {
    ...
    private final int colorText = this.getTextColorOrDefault("text", 0xFAFAFF);
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        if (mName.length() > 26) {
            ...
            for (String str : split) {
                ...
                if (total_line_length > max_chars_per_line) {
                    fontRendererObj.drawString(current_line, 10, line_counter, colorText);
                    line_counter += 8;
                    current_line = "";
                    index = 0;
                    total_line_length = str.length();
            ...
            fontRendererObj.drawString(mName, 10, line_counter, colorText);
            line_counter += 8;
        }
        ...
        if (mContainer != null) {//(mWrench ? 0 : 1) | (mScrewdriver ? 0 : 2) | (mSoftHammer ? 0 : 4) | (mHardHammer ? 0 : 8) | (mSolderingTool ? 0 : 16) | (mCrowbar ? 0 : 32) | (mMachine ? 0 : 64));
            if ((mContainer.mDisplayErrorCode & 1) != 0) {
                fontRendererObj.drawString(GT_Utility.trans("132", "Pipe is loose."), 10, line_counter, colorText);
                line_counter += 8;
            }

            if ((mContainer.mDisplayErrorCode & 2) != 0) {
                fontRendererObj.drawString(GT_Utility.trans("133", "Screws are loose."), 10, line_counter, colorText);
                line_counter += 8;
            }

            if ((mContainer.mDisplayErrorCode & 4) != 0) {
                fontRendererObj.drawString(GT_Utility.trans("134", "Something is stuck."), 10, line_counter, colorText);
                line_counter += 8;
            }

            if ((mContainer.mDisplayErrorCode & 8) != 0) {
                fontRendererObj.drawString(GT_Utility.trans("135", "Platings are dented."), 10, line_counter, colorText);
                line_counter += 8;
            }

            if ((mContainer.mDisplayErrorCode & 16) != 0) {
                fontRendererObj.drawString(GT_Utility.trans("136", "Circuitry burned out."), 10, line_counter, colorText);
                line_counter += 8;
            }

            if ((mContainer.mDisplayErrorCode & 32) != 0) {
                fontRendererObj.drawString(GT_Utility.trans("137", "That doesn't belong there."), 10, line_counter, colorText);
                line_counter += 8;
            }

            if ((mContainer.mDisplayErrorCode & 64) != 0) {
                fontRendererObj.drawString(GT_Utility.trans("138", "Incomplete Structure."), 10, line_counter, colorText);
                line_counter += 8;
            }

            if (mContainer.mDisplayErrorCode == 0) {
                if (mContainer.mActive == 0) {
                    fontRendererObj.drawString(GT_Utility.trans("139", "Hit with Soft Mallet"), 10, line_counter, colorText);
                    line_counter += 8;
                    fontRendererObj.drawString(GT_Utility.trans("140", "to (re-)start the Machine"), 10, line_counter, colorText);
                    line_counter += 8;
                    fontRendererObj.drawString(GT_Utility.trans("141", "if it doesn't start."), 10, line_counter, colorText);
                    line_counter += 8;
                } else {
                    fontRendererObj.drawString(GT_Utility.trans("142", "Running perfectly."), 10, line_counter, colorText);
                    line_counter += 8;
                }
                if (mContainer.mTileEntity.getMetaTileEntity() instanceof GT_MetaTileEntity_DrillerBase) {
                    ...
                    if (tItem == null || !GT_Utility.areStacksEqual(tItem, GT_ModHandler.getIC2Item("miningPipe", 1L))) {
                        fontRendererObj.drawString(GT_Utility.trans("143", "Missing Mining Pipe"), 10, line_counter, colorText);
                    }
                    ...
                    if (tItem == null || !(tItem.getItem() == GT_MetaGenerated_Tool_01.INSTANCE && tItem.getItemDamage() >= 170 && tItem.getItemDamage() <= 177)) {
                        fontRendererObj.drawString(GT_Utility.trans("144", "Missing Turbine Rotor"), 10, line_counter, colorText);
                    }
                }
            }
        }
