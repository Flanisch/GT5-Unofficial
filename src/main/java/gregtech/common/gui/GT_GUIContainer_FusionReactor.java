...
public class GT_GUIContainer_FusionReactor extends GT_GUIContainerMetaTile_Machine implements NEI_TransferRectHost {
    ...
    private final int
        ...
        colorTextValue = this.getTextColorOrDefault("value", 0xFF0000);
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        fontRendererObj.drawString(mName, 8, -10, colorTextTitle);

        if (mContainer != null) {
            if ((mContainer.mDisplayErrorCode & 64) != 0)
                fontRendererObj.drawString("Incomplete Structure.", 10, 8, colorText);

            if (mContainer.mDisplayErrorCode == 0) {
                if (mContainer.mActive == 0) {
                    fontRendererObj.drawString("Hit with Soft Mallet to (re-)start the Machine if it doesn't start.", -70, 170, colorText);
                } else {
                    fontRendererObj.drawString("Running perfectly.", 10, 170, colorText);
                }
            }
            if(this.mContainer.mEnergy > 160000000 && this.mContainer.mEnergy < 160010000)
                fontRendererObj.drawString("160,000,000 EU", 50, 155, colorTextValue);
            else if(this.mContainer.mEnergy > 320000000 && this.mContainer.mEnergy < 320010000)
                fontRendererObj.drawString("320,000,000 EU", 50, 155, colorTextValue);
            else if(this.mContainer.mEnergy > 640000000 && this.mContainer.mEnergy < 640010000)
                fontRendererObj.drawString("640,000,000 EU", 50, 155, colorTextValue);
            else if(this.mContainer.mEnergyLong > 5120000000L && this.mContainer.mEnergyLong < 5120080000L)
                fontRendererObj.drawString("5,120,000,000 EU", 50, 155, colorTextValue);
            else
                fontRendererObj.drawString(GT_Utility.formatNumbers(this.mContainer.mEnergyLong) + " EU", 50, 155, colorTextValue);
        }
    }
